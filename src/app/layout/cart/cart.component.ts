import { Component, OnInit, ɵChangeDetectionSchedulerImpl } from '@angular/core';
...
export class CartComponent implements OnInit {
  ...
  addOrder() {
    if (this.items.length > 0) {
      let data = {
        ...
        items: this.items,
        totalAmount: this.sum(this.items),
  ...
  onPayment() {
    ...
    this.service.addOrder(this.localStorage.userToken, orderData).subscribe((response: any) => {
      if (response.success === true) {
        let data = {
          price: this.sum(this.items),
          userID: this.localStorage.userID,
  ...
  createForm() {
    this.form = new FormGroup({
      ...
      address: new FormControl(this.localStorage.userAddress, Validators.required),
      city: new FormControl(this.localStorage.userCity, Validators.required),  // فقط label
      state: new FormControl(this.localStorage.userState, Validators.required), // فقط label
      fullName: new FormControl(this.localStorage.userFullName, Validators.required),
  ...
  editUser() {
    if (this.form.controls.fullName.value == null) {
      this.messageService.add({ severity: 'error', summary: ' نام و نام خانوادگی را وارد کنید.' });
    } else if (this.form.controls.address.value == null) {
      this.messageService.add({ severity: 'error', summary: ' آدرس را وارد کنید.' });
    } else if (this.form.controls.state.value == null) {
      this.messageService.add({ severity: 'error', summary: ' استان را انتخاب کنید.' });
    } else if (this.form.controls.city.value == null) {
      this.messageService.add({ severity: 'error', summary: ' شهرستان را انتخاب کنید.' });
    } else if (this.form.controls.postalCode.value == null || this.form.controls.postalCode.value.length !== 10) {
      this.messageService.add({ severity: 'error', summary: ' کد پستی باید ۱۰ رقمی باشد.' });
    } else {
      this.service.editUser(this.localStorage.userToken, this.localStorage.userID, this.form.value).subscribe((response: any) => {
        if (response.success === true) {
          this.active++;
        } else {
          this.messageService.add({ severity: 'error', summary: ' ثبت نشد ' });
        }
      });
    }
  }
  ...
  cityOnChange(e: any) {
    this.form.controls['city'].setValue(e.value['label']);
  }

  stateOnChange(e: any) {
    this.form.controls['state'].setValue(e.value['label']);
    this.cities = [];
